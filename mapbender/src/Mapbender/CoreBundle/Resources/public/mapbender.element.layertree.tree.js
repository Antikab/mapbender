(function ($) {
  $.widget('mapbender.mbLayertree', $.mapbender.mbDialogElement, {
    ...
    _setup: function (mbMap) {
      ...
      const hasNonPersistentScrollbars =
        navigator.userAgent.indexOf('Mac') >= 0 ||
        navigator.userAgent.indexOf('Firefox') >= 0;
      if (
        hasNonPersistentScrollbars &&
        this.element.closest('.sideContent').length
      ) {
        this.element
          .closest('.container-accordion')
    ...
    _createTree: function () {
      ...
      for (var i = sources.length - 1; i > -1; i--) {
        if (
          this.options.showBaseSource ||
          !sources[i].configuration.isBaseSource
        ) {
          var source = sources[i];
          var $sourceNode = this._createSourceTree(sources[i]);
          var themeOptions =
            this.options.useTheme && this._getThemeOptions(source.layerset);
          if (themeOptions) {
    ...
    _createEvents: function () {
      ...
      this.element.on(
        'click',
        '.-fn-toggle-info:not(.disabled)',
        this._toggleInfo.bind(this)
      );
      this.element.on(
        'click',
        '.-fn-toggle-children',
        this._toggleFolder.bind(this)
      );
      this.element.on(
        'click',
        '.-fn-toggle-selected:not(.disabled)',
      ...
      $(document).bind(
        'mbmapsourceloadstart',
        $.proxy(self._onSourceLoadStart, self)
      );
      $(document).bind(
        'mbmapsourceloadend',
        $.proxy(self._onSourceLoadEnd, self)
      );
      $(document).bind(
        'mbmapsourceloaderror',
        $.proxy(self._onSourceLoadError, self)
      );
      $(document).bind('mbmapsourceadded', $.proxy(self._onSourceAdded, self));
      $(document).bind(
        'mbmapsourcechanged',
        $.proxy(self._onSourceChanged, self)
      );
      $(document).bind(
        'mbmapsourceremoved',
        $.proxy(self._onSourceRemoved, self)
      );
      $(document).bind(
        'mbmapsourcelayerremoved',
        $.proxy(this._onSourceLayerRemoved, this)
      );
    ...
    _findThemeNode: function (layerset) {
      return $(
        'ul.layers:first > li[data-layersetid="' + layerset.id + '"]',
        this.element
      );
    },
    _createLayerNode: function (layer) {
      ...
      if (
        ...
        (treeOptions.allow.toggle || treeOptions.toggle)
      ) {
        if (
          this.options.hideSelect &&
          treeOptions.selected &&
          !treeOptions.allow.selected
        ) {
    ...
    _onSourceLayerRemoved: function (event, data) {
      ...
      var $node = $(
        '[data-sourceid="' + sourceId + '"][data-id="' + layerId + '"]',
        this.element
      );
      $node.remove();
    },
    _redisplayLayerState: function ($li, layer) {
      ...
      if (layer.state.outOfBounds) {
        tooltipParts.push(
          Mapbender.trans('mb.core.layertree.const.outofbounds')
        );
      }
    ...
    _getSourceNode: function (sourceId) {
      return $(
        '.serviceContainer[data-sourceid="' + sourceId + '"]',
        this.element
      );
    },
    ...
    _toggleFolder: function (e) {
      ...
      if (
        layer &&
        (!layer.children || !layer.options.treeOptions.allow.toggle)
      ) {
        return false;
      }
    ...
    _updateFolderState: function ($node) {
      ...
      $node
        .children('.leaveContainer')
        .children('.-fn-toggle-children')
        .children('i')
    ...
    _toggleSelected: function (e) {
      ...
      var themeId =
        !source && $target.closest('.themeContainer').attr('data-layersetid');
      if (themeId) {
    ...
    _initDimensionsMenu: function ($element, menu, dims, source) {
      ...
      $.each(dims, function (idx, item) {
        ...
        $('.layer-dimension-bar', menu).toggleClass(
          'hidden',
          item.type === 'single'
        );
        ...
        if (item.type === 'single') {
          ...
          dragHandlers.push(
            new Dragdealer($('.layer-dimension-bar', $control).get(0), {
              x:
                dimHandler.getStep(
                  dimData[dimDataKey].value || dimHandler.getDefault()
                ) / dimHandler.getStepsNum(),
              horizontal: true,
              vertical: false,
              speed: 1,
          ...
          Mapbender.error(
            'Source dimension ' + item.type + ' is not supported.'
          );
        }
    ...
    reIndent_: function ($lists, recursive) {
      for (var l = 0; l < $lists.length; ++l) {
        var list = $lists[l];
        var $folderToggles = $(
          '>li >.leaveContainer .-fn-toggle-children',
          list
        );
        // If all folder toggles on this level of the tree are placeholders,
        // "unindent" the whole list.
        if (
          $folderToggles.filter('.disabled-placeholder').length ===
          $folderToggles.length
        ) {
